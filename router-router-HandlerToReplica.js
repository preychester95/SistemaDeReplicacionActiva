...
frontend.on('message', function(idHandler, request) {
	...
	console.log('\n' + request.idRequest + ': Recibida peticion desde manejador '+ idHandler);
	// Send the petition through backend router, to the chosen handler:
	var idReplica = request.idReplica;
	console.log('\n' +'Enviando peticion a la replica ' + idReplica);
...
backend.on('message', function(idReplica, reply) {
	...
	console.log('\n' + reply.idRequest +": Recibida respuesta de la replica, enviando al manejador "+reply.idHandler);
	// Send the response 
